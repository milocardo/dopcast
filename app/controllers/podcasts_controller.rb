...
class PodcastsController < ApplicationController
  def index
    ...
    if params[:query][:types].include?("episode") || params[:query][:types] == [""]
      @episodes = HTTParty.get( "https://listennotes.p.mashape.com/api/v1/search?type=episode&q=#{params[:query][:q]}",
        headers:{
          "X-Mashape-Key" => ENV['FULL_SEARCH_KEY'],
          "Accept" => "application/json"
        }
    ...
    # if params.dig(:query, :q).present?
    #   # Search all the podcasts with the query params
    #   @podcasts = HTTParty.get(
    #    "https://listennotes.p.mashape.com/api/v1/search?type=podcast&q=#{params[:query][:q]}",
    #     headers: {
    #       "X-Mashape-Key" => ENV['FULL_SEARCH_KEY'],
    #       "Accept" => "application/json"
    #     }
  ...
  def query_params
    params.require(:query).permit(:q, :genre_ids, :language, :len_min, :len_max)
  end
end
